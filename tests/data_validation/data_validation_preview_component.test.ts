...
import { LIGHT_HIGHLIGHT_COLOR } from "../../src/constants";
...
describe("Data validation preview", () => {
  ...
  test("Ranges of hovered previews are highlighted", async () => {
    ...
    expect(model.getters.getHighlights()).toMatchObject([
      { zone: toZone("A1"), noninteractive: true, color: LIGHT_HIGHLIGHT_COLOR },
      { zone: toZone("A3"), noninteractive: true, color: LIGHT_HIGHLIGHT_COLOR },
    ]);
